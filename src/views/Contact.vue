<template>
  <div class="cry-contact cry-box">
      <h1 class="cry-box-title">feel free to contact us</h1>
      <div class="cry-box-content">
        contact:
        <ul>
          <li><a href="mailto:tholuta@yahoo.com">tholuta@yahoo.com</a></li>
          <li><a href="https://github.com/cryw0rks">github</a></li>
        </ul>
        <br>
        social media:
        <ul>
          <li><a href="https://facebook.com/cryw0rks" target="_blank">facebook</a> (not active)</li>
          <li><a href="https://twitter.com/cryw0rks" target="_blank">twitter</a> (not active)</li>
          <li><a href="https://instagram.com/cryw0rks" target="_blank">instagram</a> (not active)</li>
        </ul>
        <br><br>
      </div>
  </div>
</template>

